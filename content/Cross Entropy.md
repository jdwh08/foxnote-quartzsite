...
thus for binary classification, we get
- $=-(y log(p) + (1-y) log(1-p))$
(we treat this as a [[Bernoulli Trials and Random Variables|Bernoulli]])
- We can also vectorize this: $-y^T\log(p)-(1-y)^T\log(1-h)$
